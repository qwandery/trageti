...
### v0.3 Amendment â€” 2026-06-01 â€” Baseline migration reset
...
CHECK, `created_at` columns, `trageti_fulltext` with sync triggers,
...
databases. Repository write paths generate canonical `new Date().toISOString()`
timestamps; baseline DDL defaults and `trageti_schema_version.applied_at` use
SQLite `datetime('now')`.

---

### v0.3 Rev2 Amendment - 2026-06-05 - RRF scorer contract and implementation alignment

This rev2 specification is a targeted copy of the v0.3 spec with two alignment
goals:

1. Correct stale language where the written spec drifted from the current
   implementation.
2. Incorporate the latest v0.3 scorer revision from
   `_docs/trageti-rrf-scorer-brief-rev1.md`.

The scorer revision is part of the latest v0.3 contract even where source code
has not yet caught up. It supersedes earlier `RetrievalScorer` /
...
The `LibraryTable` union â€” and therefore `ColumnExtension.table` â€” becomes
...
The lineage is: `trl_fts_config` (spec, never built) â†’ `trl_fts_meta` (v003
code) â†’ `trageti_tokenizer` (this amendment, steady state).
...
lives in the `trageti_namespaces.embedding_table` column â€” no code re-derives
...
- **Migration v004 â€” canonical timestamp backfill.** A standard
  (non-FK-toggle) migration that rewrites existing `created_at` columns to
  canonical ISO-8601 (`strftime('%Y-%m-%dT%H:%M:%fZ', â€¦)`) so the determinism
  tie-break (`createdAt ASC`) holds on upgraded databases. v0.3 repositories
  also generate `new Date().toISOString()` for new rows.
- **Migration v005 â€” table rename.** An FK-toggle migration that renames the
...
run â€” a mid-run failure never strands the version history.

**Embedding vec0 tables â€” eager, best-effort.** Migration v005 renames each
...
new vec0 table, copies rows via `INSERT â€¦ SELECT`, repoints `embedding_table`,
...
lazy runtime rename path exists â€” the rename is entirely contained in v005.

**Compatibility statement.** Fresh v0.3 databases, and migrated databases
opened with `sqlite-vec` loaded, contain only `trageti_*` library tables â€”
zero `trl_*` tables remain. The sole exception is the remote, unsupported
case above.

---

### v0.3 Amendment â€” 2026-05-20 â€” API-conformance remediation (R9)
...
- **`GraphAdapterTraversalOptions`** (adapter-facing, new) =
  ...
  interface's option type â€” the field set is unchanged.
...
`appliedAt` is `string | null` â€” the ISO-8601 timestamp recorded in
...
- `RETRIEVAL_INVALID_TEMPORAL_WINDOW`, `RETRIEVAL_INVALID_CONFIDENCE`,
  `RETRIEVAL_INVALID_TOKEN_BUDGET` â€” `RetrievalInputError` codes for invalid
  retrieval / context-assembly inputs that were previously conflated under
  `RETRIEVAL_INPUT_EMPTY`.
- `SCORER_BATCH_LENGTH_MISMATCH` â€” `RetrievalInputError`, thrown when a
  `IRetrievalScorer.scoreBatch()` implementation returns a score array whose
  length differs from the candidate batch (previously a generic
  `ValidationError`).

**5. `includeSuperseded` retrieval semantics â€” explicit.** By default
...
assertion with `valid_from <= anchor` regardless of `valid_until` â€” closed and
superseded versions included â€” subject to all other filters. This same
...
**6. FTS5 tokenizer â€” `trustedCustomTokenizer`.** `FTS5TokenizerConfig` carries
...
**7. Extension identifier rules â€” reserved keywords permitted.** Extension
...
happens to be a SQLite reserved keyword is **accepted** â€” every identifier is
...
`RetrievalMeta.queryTextMode` is `QueryTextMode | null` â€” `null` when the call
...
### v0.3 Amendment â€” 2026-05-21 â€” Retrieval & graph polish
...
`validFrom <= atPosition` regardless of `validUntil` â€” versions closed before
`atPosition` included â€” mirroring `retrieve()`'s temporal relaxation. The
...
before the optional `graph-expand` / `trajectory-expand` steps â€” the order in
...
is **10**; retrieval over-fetches `limit Ã— 3` candidates before scoring and
...
(default `CTEGraphAdapter`) returns links in a stable order â€” shallowest
traversal depth, then link `created_at`, then link `id` â€” so repeated calls
...
- **`getStats()` and `getPendingIndexing()` semantics** for the four
  observable `(sqlite-vec loaded Ã— vec0 exists)` states; `vectorReady`
  field on `NamespaceStats`.
- **Rev2 scorer contract.** `RRFScorer` is the default scorer. The previous
  weighted-linear default is retained as `LinearScorer`; its formula matches
  the actual v0.2 code (position-range-normalized recency; weight
  renormalization when BM25 or vector signals are absent; min-max BM25
  normalization with `range === 0 â†’ 1` rule). `semanticDistance` remains
  nullable.
...
- `deleteNamespace()` looks up the embedding table from `trageti_namespaces`
  rather than the in-memory cache (externally invisible correctness fix â€”
...
- `queryTextMode` ADDED with the **default kept as `'fts5'`** (preserves
  current raw FTS behavior â€” no consumer breaks on minor-version upgrade).
...
- FK enforcement (warn â†’ fail-closed); `NamespaceDimensionMismatchError`
...
#### Example 1 â€” Hybrid retrieval (vector + BM25, the default path)
...
meaningful â€” `MockEmbeddingProvider` is for tests and quickstarts only).
...
For a real RAG path, replace `MockEmbeddingProvider` with a production
`EmbeddingProvider` implementation. The optional `validUntil`, `supersedesId`,
`entityId`, and `entityType` fields on `writeAssertion()` are omitted in this
example â€” they default to `null` via the input normalization step inside
`writeAssertion()` (see `NewAssertionInput` in Core Concepts).

#### Example 2 â€” BM25-only retrieval (no `sqlite-vec` required)
...
const store = await TemporalStore.create({
  ...
  // No embeddingDimension, no embeddingProvider â€” namespace is vectorless.
});
  ...
  citations: [
    {
      id: 'c-1',
      episodeId: 'ep-1',
      sourceRef: 'waiver#Â§3.1',
      excerpt: 'waives liability for ordinary negligence',
    },
  ],
});
// indexBatch is NOT called â€” vectorless namespaces don't index vectors.
...
// Equivalent path via hybrid fallback (no provider + queryText only â†’ BM25):
//   retrievalStrategy: 'hybrid' (or omitted â€” hybrid is the default)
...
interface Episode {
  ...
  position: number;
  /** ISO 8601 â€” real-world time of the event; display and audit only.
   *  Never used for ordering or retrieval filtering â€” that is what
   *  position is for. */
  occurredAt: string;
...
any repository write. Validators receive the normalized shape â€” `validUntil`,
...
treated as `null`." Do not say "undefined and null" â€” the repo's
...
interface AssertionCitation {
  id: string;
  /** FK â†’ trageti_assertions.id */
  assertionId: string;
  /** FK â†’ trageti_episodes.id (must share the assertion's namespace) */
  episodeId: string;
...
interface AssertionLink {
   ...
   *  'deepens', 'qualifies', 'contextualizes', 'contradicts', 'measures' â€”
   *  but any string is allowed. */
  linkType: string;
  validFrom: number;
  validUntil: number | null;
  /** FK â†’ trageti_episodes.id (must share the link's namespace). */
  sourceEpisodeId: string;
...
Scorers receive every candidate's semantic distance (nullable in BM25-only
mode), optional BM25 score, candidate position, and scoring context. Rev2 makes
batch scoring the only scorer interface because rank-based fusion and
cross-candidate normalization both require the full candidate set.
...
interface ScoringContext {
   ...
   *  zero active assertions, both fields are null; a null or degenerate
   *  range collapses recency to 1 per the LinearScorer contract. */
  namespacePositionRange: { min: number | null; max: number | null };
...
**Contract for `IRetrievalScorer.scoreBatch()`:**
...
- Returned scores MUST be finite numbers. `NaN` and `Â±Infinity` throw
  `RetrievalInputError`.
...
- The same `(candidates, context)` input MUST produce identical output across
  ...
  required.
- The retrieval pipeline always calls `scoreBatch()`; individual `score()` is
  not part of `IRetrievalScorer`.

**Default scorer (`RRFScorer`).** Rev2 makes Reciprocal Rank Fusion the default
scorer. `RRFScorer` takes `constructor(options?: { k?: number;
includeRecency?: boolean })`; defaults are `k = 60` and
`includeRecency = true`.

RRF computes a score for each candidate using one term per included ranker:

```text
RRF_score(d) = SUM(1 / (k + rank_r(d)))
```

The rankers are semantic distance ascending (`semanticDistance`: lower is
better), BM25 score ascending (`bm25Score`: raw FTS5 scores are negative, and
more-negative is better), and recency by position descending (`position`:
higher is more recent) when `includeRecency` is true. Ranks are 1-based. A
candidate missing a signal contributes `0` from that ranker. Recency is present
for every candidate unless `includeRecency` is false.

**Weighted-linear scorer (`LinearScorer`).** `LinearScorer` is the renamed
version of the previous default weighted-linear scorer. It is retained as a
shipped alternative for consumers who want explicit signal weights:
`{ semantic; keyword; recency }`. A `LinearScorer` class may expose a public
`score(candidate, context)` convenience method, but that method is not part of
`IRetrievalScorer` and the library never calls it.
...
`LinearScorer.score(candidate, context)` covers four convenience cases:
...
D. both null  (unreachable â€” pipeline filters before scoring):
   throw TragetiError({
...
`LinearScorer.scoreBatch(candidates, context)` mirrors the per-candidate cases but
...
values in the batch are equal â€” including the trivial case of a single
non-null `bm25Score` â€” `range = max - min === 0`. Every such candidate is
assigned `bm25Norm = 1`. This matches the existing implementation and is part
of the `LinearScorer` contract; custom scorers replacing `LinearScorer` SHOULD
follow the same rule for consistency, though the contract only requires
...
`entityType`) to `null`. Validators may rely on this â€” they will never see
...
VIRTUAL TABLE ... USING fts5(..., tokenize='<config>')` DDL exactly once,
  at the moment a brand-new `trageti_fulltext` table is created â€” never against a
...
power. v0.3 clarifies that raw schema SQL is **trusted code** â€” never an
...
- `referencesNamespace: true` without `namespaceColumn` â†’ `SchemaExtensionError`.
...
`deleteNamespace()`, regardless of options â€” only per-namespace rows are
...
output, intended for tests and quickstarts only â€” explicitly _not_ suitable for
...
The `EmbeddingProvider` interface is the v0.3 contract. External reference
adapters (Ollama, Transformers.js, OpenAI) are future or non-contract package
surfaces; they are not current core exports. If shipped later, they may appear
as subpath exports, companion packages, or both, and may change distribution
shape without breaking the v0.3 core contract. Adopters who need stable adapter
import paths should pin to the specific package they consume, or implement the
`EmbeddingProvider` interface themselves.

Reference adapters discussed for future packaging:

- `OllamaEmbeddingProvider` â€” local Ollama HTTP server.
- `TransformersJsEmbeddingProvider` â€” in-process via `@xenova/transformers`.
- `OpenAIEmbeddingProvider` â€” OpenAI-compatible HTTP API.
...
item failed â€” that information is inherently lost across the call boundary.
...
- **`'fail-fast'` (default).** The library calls `provider.embed(texts)`
  ...
  No `IndexBatchResult` is returned in this mode â€” the partial-count
...
- **`'skip'`.** The library iterates the input one item at a time, calling
  ...
  raw `Error` object, NOT a stack trace, and NOT the underlying message â€”
  ...
  trade-off is that `'skip'` is roughly NÃ— slower than `'fail-fast'` for
...
onProviderError: \'fail-fast\' to surface the cause' }`. The previous
    live index is preserved unchanged. This is the safe default â€” a
...
opt-in is the documented way to relax it â€” and the only way.
...
- All log codes use the `TRGT_` prefix and `SCREAMING_SNAKE_CASE` (e.g.
  `TRGT_RETRIEVE_VECTOR_SKIPPED`). Codes are stable across patch releases and
...
- Stored namespace is vector-configured AND the caller supplies a matching
  `embeddingDimension` (or `embeddingProvider.dimension`) â†’ bind and
  proceed.
- Stored namespace is vector-configured AND the caller supplies a
  non-matching dimension â†’ throw `NamespaceDimensionMismatchError`.
- Stored namespace is vector-configured AND the caller supplies neither
  `embeddingDimension` nor `embeddingProvider` â†’ bind and proceed. The
  ...
  - **Caller-supplied vectors** â€” `indexAssertion(id, embedding)`,
  ...
  - **Provider-derived indexing** â€” calling
    ...
    instead.
  - **Provider-derived retrieval** â€” `retrieve({ queryText, ... })` with no
...
- Stored namespace is vectorless AND the caller supplies neither â†’ bind
  and proceed (vectorless reopen).
- Stored namespace is vectorless AND the caller supplies dimension or
  provider â†’ throw `NamespaceDimensionMismatchError` with the actionable
  message pointing at `upgradeNamespaceToVector()` (no auto-upgrade â€” see
  Initialization).

Vectorless â†’ vector-configured upgrade is supported via an explicit upgrade
...
### Migration v003 â€” Optional vector storage
...
interface Migration {
   ...
   *  COMMIT â€” the FK-toggle runner owns the transaction. */
...
quiesced â€” no other connection should hold an active transaction during their
run.
...
- **vector-configured namespace** â€” has either `embeddingDimension` or an
  ...
  both-non-null.
- **vectorless namespace** â€” neither `embeddingDimension` nor
  `embeddingProvider` configured. Both `embedding_dimension` and
  `embedding_table` columns are NULL. The CHECK enforces both-null.
- **vector-ready namespace** â€” vector-configured AND `sqlite-vec` is loaded
...
table itself has been created â€” it is the _planned_ table name, not proof of
existence (see Maintenance â†’ `getStats`/`getPendingIndexing`). This is what
...
interface CreateOptions extends TemporalStoreOptions {
   ...
   *    to `true` â€” closing the store closes the handle trageti owns.
   *  - When `database` is a Database instance (caller opened the handle),
   *    defaults to `false` â€” caller retains ownership and is responsible
   *    for closing it.
   *
...
ensureVectorReady(namespace):
  ...
  - If the vec0 table for the namespace does not exist:
    ...
    encounters that impossible state, it throws an internal error â€”
...
NOT called by: writes (writeEpisode, writeAssertion, writeCitation, writeLink),
              ...
              retrieve Step 3 (keyword re-ranking â€” FTS-based, not vector),
...
performs the upgrade â€” it only operates on already-configured namespaces.
...
(see Core Concepts â†’ Assertion); structural invariants, configured
...
interface IndexBatchResult {
   ...
   *  errorCode is a short sanitized stable identifier â€” never a raw
...
of silent (the v0.2 behavior of silently dropping unknown IDs is removed â€”
...
`(sqlite-vec loaded Ã— vec0 exists)` states are documented under
Maintenance â†’ `getStats` and `getPendingIndexing` below.
...
interface RetrievedAssertion extends Assertion {
  /** Final score from the configured IRetrievalScorer; higher = better. */
  score: number
...
- **`'hybrid'` (default).** At least one of `queryEmbedding` or non-empty
  ...
  missing â€” see the degradation rules under Retrieval Implementation below.
...
`RETRIEVAL_REQUIRES_VECTOR_BACKEND` code â€” that condition raises
...
`getTemporalSnapshot` takes a `TemporalSnapshotOptions` object â€”
`{ namespace; atPosition; entityTypes?; assertionTypes?; includeSuperseded? }`
...
`validUntil` â€” versions closed before `atPosition` included â€” subject to the
...
None call `ensureVectorReady()` â€” they are pure SQL paths over `trageti_assertions`
...
interface ReindexOptions {
   ...
   *  ReindexResult.skipped entries â€” without it, a partial staging build
...
interface ReindexResult {
   ...
   *  sanitization rules as IndexBatchResult.skipped â€” errorCode is a
...
// Four valid shapes:
//   {}                                        â†’ vectorless namespace
//   { embeddingDimension }                    â†’ vector-configured (manual vectors)
//   { embeddingProvider }                     â†’ vector-configured (dimension from provider)
//   { embeddingDimension, embeddingProvider } â†’ vector-configured (the two MUST agree)
...
`embedding_dimension`, `embedding_table`, `created_at`, and `config` â€”
...
- For an **existing vector-configured namespace**: re-calling
  ...
  - Neither `embeddingDimension` nor `embeddingProvider` is supplied â€” no
...
- For an **existing vectorless namespace**: re-calling `initNamespace()`
  ...
  â†’ vector upgrade requires `upgradeNamespaceToVector()`, not a re-call
  of `initNamespace()`).
...
standard `ensureVectorReady()` contract â€” `upgradeNamespaceToVector` does
...
dropped**, regardless of `cascade` â€” they may hold rows for other
...
`ReferencedExtensionTableError` listing them. The deletion is transactional â€”
...
stored CREATE statement â€” SQL DDL parsing is brittle and version-dependent.
...
contents â€” there is no in-place tokenizer upgrade. With a large corpus
...
#### `getStats()` and `getPendingIndexing()` â€” vec0-state matrix

Recall: `vectorReady â‡” (sqlite-vec loaded) AND (vec0 exists for this
...
```text
getStats(namespace): Promise<NamespaceStats>  â€” never calls ensureVectorReady().
...
getPendingIndexing(namespace): Promise<Array<{ id, content }>>
                                â€” never calls ensureVectorReady(). Like every
  ...
  - sqlite-vec NOT loaded + vec0 missing: returns ALL active assertions.
                                          ...
                                          case above â€” vec0 truly does not
...
migration â€” a flat record describing version, name, and applied timestamp.
...
Routing for retrievalStrategy: 'vector'
  ...
  be obtained â€” caller supplied queryEmbedding directly, or queryText +
...
- `MissingPeerDependencyError` (`sqlite-vec` not loaded) â€” caught; emit
...
- `RetrievalInputError(RETRIEVAL_NAMESPACE_VECTORLESS)` (vectorless
  namespace) â€” caught; emit `TRGT_RETRIEVE_VECTOR_SKIPPED` with
  `reason: 'NAMESPACE_VECTORLESS'`; route to Step 2-bm25.

The third documented `reason` value, `'NO_PROVIDER'`, is emitted by Step 0
(not Step 2) per the routing table above â€” Step 2 never runs in that case.
...
never loads
`sqlite-vec` and never touches vec0 â€” it is the path that supports the
...
Call the configured `IRetrievalScorer.scoreBatch()` for the full candidate set.
Rev2 has no per-candidate scorer fallback. The default `RRFScorer` fuses ranks
from semantic, BM25, and optional recency signals; `LinearScorer` is available
for the previous weighted-linear behavior.
...
choice provided the scorer itself is deterministic per the `IRetrievalScorer`
contract. The contract is part of the public API: regulated-domain consumers
may rely on it for audit reproducibility. Repository-created `createdAt` values
use canonical ISO 8601 strings with consistent precision, so lexicographic order
matches temporal order for normal library writes; rows inserted outside the
repository path must preserve that property if callers rely on deterministic
timestamp ordering. The final `id ASC` tie-break assumes SQLite's default BINARY
text collation; trageti managed IDs and ordering columns must not be declared
with `COLLATE NOCASE`.
...
| `TRGT_FOREIGN_KEYS_ENABLED`          | debug    | `DefaultConnectionVerifier` â€” emitted once on successful enablement. The failure path is a thrown `ConnectionVerificationError`, NOT a log.                                                                                                                                                                                                |
...
| `TRGT_RETRIEVE_VECTOR_TABLE_MISSING` | debug    | vector-readable checks when a namespace's planned embedding table is not present                                                                                                                                                                                                                                                             |
...
| `TRGT_DEPRECATED_USAGE`              | warn     | `store.advanced.closeAssertion`; once per process per symbol (suppressible via `Logger`)                                                                                                                                                                                                                                                     |
...
production telemetry â€” production observability flows through `Metrics`.
...
error type for "sqlite-vec not loaded" â€” there is no
...
| `RETRIEVAL_INVALID_QUERY_TEXT`      | `queryTextMode: 'fts5'` with a `queryText` that is not a valid FTS5 expression. The thrown message is generic â€” it never echoes the query text or a raw SQLite parser fragment. _(Added by the 2026-05-21 amendment.)_ |
...
| `SCORER_BATCH_LENGTH_MISMATCH`      | an `IRetrievalScorer.scoreBatch()` implementation returns a score array whose length differs from the candidate-batch length. _(Added by the R9 amendment.)_                                                             |
| `SCORER_INVALID_OUTPUT`             | an `IRetrievalScorer.scoreBatch()` implementation returns `NaN`, `Infinity`, or `-Infinity`.                                                                                                                             |
...
| Code                      | Raised by                                                                                                                                                                                                                                                                         |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SCORER_NO_USABLE_SIGNAL` | `LinearScorer` (and any custom scorer following the same contract) when a candidate reaches scoring with both `semanticDistance` and `bm25Score` null â€” indicates the pipeline failed to filter unscorable candidates before Step 5.                                            |
| `INTERNAL_INVARIANT`      | A generic "this should never happen" guard tripped â€” e.g. a row not found immediately after the library itself inserted it within the same transaction. Indicates a library bug or external interference with library tables. _(Added by the v0.3 Table-naming Amendment, R7.)_ |
...
- **Multi-instance namespace lifecycle.** Deleting a namespace created by
  another store instance â€” verifies the v0.3 DB-authoritative
...
- **`getStats` / `getPendingIndexing` matrix.** All four observable
  `(sqlite-vec loaded Ã— vec0 exists)` states for a vector-configured
...
- **Hybrid retrieval graceful degradation.** `queryText` only with no
  provider â†’ BM25 fallback emits `TRGT_RETRIEVE_VECTOR_SKIPPED` with
  `reason: 'NO_PROVIDER'`. `queryText` only with no `sqlite-vec` â†’
  fallback emits with `reason: 'NO_SQLITE_VEC'`. `queryEmbedding` supplied
  with no `sqlite-vec` â†’ throws `MissingPeerDependencyError`.
- **FK-toggle migration runner failure paths.** Migration body throws,
  `foreign_key_check` returns rows, COMMIT fails â€” in each case,
...
- **FTS adversarial inputs.** Quotes, parens, FTS5 operators (`AND`, `OR`,
  ...
  whitespace-only strings â€” all in BOTH `queryTextMode: 'phrase'` (default)
...
- **`writeAssertion()` normalization order.** Custom validator receives
  ...
  `undefined`.
- **`RRFScorer` rank fusion.** Semantic, BM25, and optional recency ranks
  produce deterministic scores with default `k = 60`; missing semantic/BM25
  signals contribute zero for that ranker.
- **`LinearScorer` compatibility.** Covers the previous weighted-linear
  hybrid/vector-only/BM25-only cases, the both-null failure case, and
  `scoreBatch()` `range === 0` BM25 normalization.
...
- **Determinism contract.** Same query against same DB state returns same
  ...
  `(score DESC, validFrom DESC, createdAt ASC, id ASC)`, including
  repository-created canonical `createdAt` string ordering and BINARY `id`
...
- **FTS tokenizer metadata round-trip.** `prepareDatabase` â†’ `initNamespace` â†’
  write assertions â†’ `rebuildFts({ tokenizer })` â†’ close â†’ reopen â†’ confirm
...
Current core implementation shape plus future adapter context:
...
Optional external adapters with extra runtime dependencies are future or
non-contract package surfaces â€” see Extension Interfaces â†’ Provider Adapters
and Open Questions for the out-of-contract packaging carve-out. Adapters never
become hard runtime dependencies of the core `trageti` import.
...
- `sqlite-vec` is an **optional peer** declared with
  ...
  reaches Step 2 (vector candidate selection) â€” including a hybrid query
  that derives an embedding via a configured provider â€” requires
...
Future or companion provider adapters declare their own optional peer
dependencies. The core `trageti` import never carries these.
...
- Schema migrations are forward-only. A v0.x â†’ v1.0 migration path is
...
do no async work beyond what they did in v0.2 for writes and introspection â€”
...
```typescript
// Before (broken in v0.2 README â€” second call throws)
...
```typescript
// Before (v0.2 â€” six required-null fields per assertion)
...
// After (v0.3 â€” nullables omitted; default to null via writeAssertion's normalization)
await store.writeAssertion({
  id: 'a-1', namespace: 'demo', type: 'observation', content: '...',
...
non-undefined), so the validator body needs no changes â€” only the parameter
type annotation.
...
### Keyword querying â€” default mode flip

`queryTextMode` default flips to `'phrase'` in 0.3.0. Callers jumping
0.1.x â†’ 0.3.0 directly experience a silent default flip; pass
...
// v0.3 equivalent â€” preserve raw FTS5 semantics explicitly
await store.retrieve({ queryText: 'alpha AND beta', queryTextMode: 'fts5' /* ... */ });

// v0.3 default â€” safe phrase mode (alpha AND beta is a literal three-word phrase)
await store.retrieve({ queryText: 'alpha AND beta' /* queryTextMode: 'phrase' */ });
```

### Batch indexing â€” result envelope
...
### Reindexing â€” staging-swap default
...
- **`upgradeNamespaceToVector(name, options)`** when a vectorless
  ...
  (vectorless â†’ vector-configured is the only state transition the
...
- **Embedding-adapter packaging â€” out of v0.3 contract.** Whether the
  ...
  contract decision (see Extension Interfaces â†’ Provider Adapters). The
...
- **Quantized vectors.** sqlite-vec supports `int8` and `bit` vector
  ...
  memory at modest accuracy cost â€” meaningful for on-device deployments.
