# trageti v0.3 rev2 / 0.4.0-rev.0 - Verification Matrix

This document maps the v0.3 rev2 specification at [trageti-spec-v0.3-rev2.md](trageti-spec-v0.3-rev2.md) to the `0.4.0-rev.0` beta remediation implementation and test evidence in this repository. Every MUST-level invariant, public API surface, error code, schema-migration behavior, and required test class has a row pointing at concrete code and a test.

Updated through the `0.4.0-rev.0` beta-readiness remediation pass. The latest local release gate is green: `lint`, `typecheck`, `test:coverage` (385 unit/integration tests; 95.08 statements, 88.43 branches, 98.8 functions, 95.08 lines), `build`, and `test:e2e` (1 built-package public-surface test).

## Public API surface

| Spec §    | Public symbol                                     | Implementation                                                      | Tests                                                        |
| --------- | ------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------ |
| §create   | `TragetiStore.create(options)`                    | `src/store/TragetiStore.ts` `create`                                | `test/integration/store-lifecycle.test.ts`                   |
| §init     | `new TragetiStore(db, opts)` + `await init()`     | `src/store/TragetiStore.ts` constructor + `init`                    | `test/integration/temporal-filter.test.ts` and most suites   |
...
| §close    | `store.close()` ownership-driven                  | `src/store/TragetiStore.ts` `close`                                 | `store-lifecycle`, `store-edge-cases` (dispose + flush)      |
| §guard    | `requireNotClosed()` open-state guard             | `src/store/TragetiStore.ts` `requireNotClosed`                      | `store-lifecycle` StoreClosedError checks                    |
| §1701     | `RetrievalStrategy` routing (hybrid/vector/bm25)  | `src/store/TragetiStore.ts` `resolveQueryEmbedding` + `retrieve.ts` | `store-operations`, `no-sqlite-vec`, `semantic-retrieval`    |
...
| §1641     | `IndexBatchResult { indexed, skipped[] }`         | `src/store/TragetiStore.ts` `indexBatch`                            | `test/integration/store-operations.test.ts`                  |
| §1977     | `RebuildFtsResult` + tokenizer round-trip         | `src/store/TragetiStore.ts` `rebuildFts`                            | `store-operations` (rebuildFts)                              |
| §1994     | `NamespaceStats` v0.3 fields                      | `src/store/TragetiStore.ts` `getStats`                              | `temporal-filter`, `vectorless-namespace`                    |
| §2036     | `UpgradeNamespaceToVectorOptions`                 | `src/store/TragetiStore.ts` `upgradeNamespaceToVector`              | `store-operations`, `vectorless-namespace`                   |
| §explain  | `store.explain()` non-executing introspection     | `src/store/TragetiStore.ts` `explain`                               | `store-lifecycle`, `store-edge-cases`                        |
...
| `INDEXING_INVALID_PROVIDER_ERROR_MODE` | `IndexingError`                        | `indexBatch` public option validation                                       | `runtime-boundary-validation`                             |
...
| `RETRIEVAL_INVALID_STRATEGY`           | `RetrievalInputError`                  | `retrieve` / `explain` public option validation                             | `runtime-boundary-validation`                             |
| `RETRIEVAL_INVALID_MODE`               | `RetrievalInputError`                  | `retrieve` / `explain` public option validation                             | `runtime-boundary-validation`                             |
| `RETRIEVAL_INVALID_QUERY_TEXT_MODE`    | `RetrievalInputError`                  | `retrieve` / `explain` public option validation                             | `runtime-boundary-validation`                             |
| `RETRIEVAL_INVALID_TEMPORAL_ANCHOR`    | `RetrievalInputError`                  | retrieve/snapshot/assertion/graph temporal anchor validation                | `runtime-boundary-validation`                             |
| `RETRIEVAL_INVALID_FILTER`             | `RetrievalInputError`                  | retrieve/snapshot/graph filter-array validation                             | `runtime-boundary-validation`                             |
...
| `REINDEX_INVALID_STRATEGY`             | `ReindexError`                         | `reindexNamespace` public option validation                                 | `runtime-boundary-validation`                             |
| `REINDEX_INVALID_PROVIDER_ERROR_MODE`  | `ReindexError`                         | `reindexNamespace` public option validation                                 | `runtime-boundary-validation`                             |
...
| Runtime JS boundary validation across public read/index/reindex paths         | **covered** | `runtime-boundary-validation.test.ts`                   |
| Schema-extension read surface for assertions, episodes, links, and path links | **covered** | `schema-extension-read-surface.test.ts`                 |
...
| Built-package public package E2E journey                                      | **covered** | `test/e2e/public-package.test.ts`                       |
...
| Per-namespace provider drives indexing / reindexing                                                 | §1252-1257, §2279      | `TragetiStore` `indexAssertion`/`indexBatch`/`reindexNamespace` via `getNamespaceProvider` | `r7-conformance.test.ts`                                |
...
| `explain()` models Step-0 provider routing                                                          | §2232-2245             | `TragetiStore.explain`                                                                     | `r7-conformance.test.ts`, `store-lifecycle.test.ts`     |
| `rebuildFts()` preserves the stored tokenizer                                                       | §2088-2099, §2350-2355 | `TragetiStore.rebuildFts` + `readStoredTokenizer`                                          | `r7-conformance.test.ts`                                |
| `getStats()` emits `TRGT_STATS_VEC_NOT_INTROSPECTED`                                                | §2133, §2404           | `TragetiStore.getStats`                                                                    | `r7-conformance.test.ts`                                |
...
| Step-0 provider failures propagate as `EmbeddingProviderError`                                      | §2569, §2589           | `TragetiStore.resolveQueryEmbedding`                                                       | `r7-conformance.test.ts`                                |
...
| Dimension / provider agreement validated at registration & upgrade; invalid dimensions rejected; provider-only upgrade derives the dimension | §1254-1257, §2146-2168 | `TragetiStore.resolveVectorDimension`; `UpgradeNamespaceToVectorOptions` union | `r7-conformance.test.ts`                    |
...
| -------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| Citation-excerpt policy enforced in `writeAssertion()` — a custom `validators` array cannot bypass it                                  | R9 §1             | `TragetiStore.enforceCitationExcerptPolicy`; `DefaultAssertionValidator` opt-out flag | `r9-conformance.test.ts`                                 |
...
| `getTemporalSnapshot(options)` canonical object form; `TemporalSnapshotOptions` exported                                               | R9 §1             | `TragetiStore.getTemporalSnapshot`; `src/pipeline/snapshot.ts`                        | `r9d-conformance.test.ts`, `snapshot.test.ts`            |
...
| `MigrationDescriptor.appliedAt` (+ additive `name`/`description`/`requiresForeignKeyToggle`)                                           | R9 §3             | `MigrationRunner.getAppliedVersions`; `TragetiStore.getMigrations`                    | `r9-conformance.test.ts`                                 |
...
| Tokenizer change on reopen never silently ignored (fail-closed when populated; rebuild when empty)                                     | R9 §6, §2348      | `TragetiStore.reconcileFtsTokenizer`                                                  | `r9-conformance.test.ts`                                 |
...
| `rank` debug/explain step emitted after `score`, before `graph-expand` / `trajectory-expand`                  | Polish §3      | `retrieve.ts` `debugStep` order; `TragetiStore.explain`                      | `v03-polish.test.ts`, `r9d-conformance.test.ts` |
