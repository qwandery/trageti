...
RRF is the new default scorer for trageti, replacing the previous weighted linear combination (now shipped as `LinearScorer`). trageti's `IRetrievalScorer` interface receives candidates with raw signal values. The RRF scorer needs to:

1. Receive the full candidate set (not individual candidates) — RRF is inherently a batch operation since it needs rank positions, which only exist relative to other candidates
2. Sort candidates by semantic distance (ascending — lower distance = better) to determine semantic ranks
3. Sort candidates by BM25 score (descending for raw negative FTS5 scores — more negative = better match) to determine BM25 ranks
4. Compute RRF score per candidate using both rank positions
5. Handle candidates missing from one signal: if a candidate has no BM25 score (null — no queryText was provided), it contributes 0 from that ranker. Same for unindexed candidates with no semantic score.
6. Handle the recency signal: trageti has three signals, not two. Recency can be treated as a third ranker — sort by position descending, assign ranks, add a third RRF term.

### Interface Fit

The trageti v0.3 spec defines `IRetrievalScorer` with an optional `scoreBatch` method:
...
RRF **must** use `scoreBatch` — individual `score()` calls cannot compute RRF because rank positions are relative to the full candidate set. The `score()` method should throw or return 0 with a warning if called individually. The real implementation lives in `scoreBatch()`.
...
class RRFScorer implements IRetrievalScorer {
  ...
  score(candidate: ScoredCandidate, context: ScoringContext): number {
    throw new Error('RRFScorer requires scoreBatch — individual scoring is not supported');
  }
