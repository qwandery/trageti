...
async function makeStoreWithMiddleware(middleware: RetrievalMiddleware[]): Promise<TemporalStore> {
  ...
  await store.writeAssertion({ id: 'a-1', namespace: NS, type: 'fact', content: 'Test assertion alpha.', validFrom: 1, validUntil: null, confidence: 1, sourceEpisodeId: 'ep-1', supersedesId: null, entityId: null, entityType: null, citations: [citationFor('a-1', 'ep-1')] })
...
describe('TemporalStore — middleware', () => {
  ...
  it('global before runs before per-call before', async () => {
    ...
    await store.retrieve({ namespace: NS, queryEmbedding: VEC_A, temporalAnchor: 1, middleware: [callMw] })
  ...
  it('multiple global middlewares run in registration order (before)', async () => {
    ...
    await store.init()
    await store.writeEpisode({ id: 'ep-1', namespace: NS, position: 1, occurredAt: '', type: 'doc', content: 'c' })
    await store.writeAssertion({ id: 'a-1', namespace: NS, type: 'fact', content: 'c', validFrom: 1, validUntil: null, confidence: 1, sourceEpisodeId: 'ep-1', supersedesId: null, entityId: null, entityType: null, citations: [citationFor('a-1', 'ep-1')] })
    await store.indexAssertion('a-1', VEC_A)
    await store.retrieve({ namespace: NS, queryEmbedding: VEC_A, temporalAnchor: 1 })
