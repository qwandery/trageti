...
describe('TemporalStore — citations', () => {
  ...
  it('rollback: duplicate citation IDs cause SQLite PK violation; assertion is not written', async () => {
    ...
    const cits = db.prepare('SELECT COUNT(*) AS c FROM trageti_citations WHERE assertion_id = ?').get('a-dup-cit') as {
      c: number;
    };
  ...
  it('rollback: pre-DB validation rejects supersession on already-closed predecessor; nothing written', async () => {
    ...
    const a = db.prepare('SELECT id FROM trageti_assertions WHERE id = ?').get('a-supersede-closed') as
      | { id: string }
      | undefined;
    expect(a).toBeUndefined();
    const pred = db.prepare('SELECT valid_until FROM trageti_assertions WHERE id = ?').get('pred') as {
      valid_until: number;
    };
