...
beforeEach(async () => {
  ...
  const mk = async (
    id: string,
    opts: { type: string; entityType: string | null; conf: number },
  ) => {
...
describe('retrieve input validation — typed RetrievalInputError codes', () => {
  it('RETRIEVAL_INPUT_EMPTY when neither queryText nor queryEmbedding is given', async () => {
    await expect(store.retrieve({ namespace: NS, temporalAnchor: 5 })).rejects.toThrow(
      RetrievalInputError,
    )
  })
  ...
  it('RETRIEVAL_REQUIRES_QUERY_TEXT for bm25 strategy with no queryText', async () => {
    await expect(
      store.retrieve({ namespace: NS, retrievalStrategy: 'bm25', temporalAnchor: 5 }),
  ...
  it('RETRIEVAL_REQUIRES_VECTOR_INPUT for vector strategy with no embedding/provider', async () => {
    await expect(
      store.retrieve({ namespace: NS, retrievalStrategy: 'vector', temporalAnchor: 5 }),
    ).rejects.toThrow(RetrievalInputError)
  })

  it('RETRIEVAL_INVALID_LIMIT for a non-positive-integer limit', async () => {
    await expect(
      store.retrieve({ namespace: NS, queryText: 'foxes', limit: 0, temporalAnchor: 5 }),
    ).rejects.toThrow(RetrievalInputError)
    await expect(
      store.retrieve({ namespace: NS, queryText: 'foxes', limit: 2.5, temporalAnchor: 5 }),
    ).rejects.toThrow(RetrievalInputError)
  })
