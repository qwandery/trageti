...
describe('Episode position monotonicity (v0.2 spec invariant)', () => {
  ...
  it('accepts strictly increasing positions within a namespace', async () => {
    await store.writeEpisode({ id: 'ep-1', namespace: NS_A, position: 1, occurredAt: '2024-01-01T00:00:00Z', type: 'document', content: 'c1' })
    await expect(
  ...
  it('rejects equal positions within a namespace', async () => {
    await store.writeEpisode({ id: 'ep-1', namespace: NS_A, position: 5, occurredAt: '2024-01-01T00:00:00Z', type: 'document', content: 'c1' })
    await expect(
  ...
  it('isolates monotonicity per namespace', async () => {
    await store.writeEpisode({ id: 'ep-a1', namespace: NS_A, position: 10, occurredAt: '2024-01-01T00:00:00Z', type: 'document', content: 'a1' })
    await expect(
