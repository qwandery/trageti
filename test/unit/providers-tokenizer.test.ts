...
describe('validateTokenizer', () => {
  ...
  it('accepts safe tokenizer args', () => {
    expect(() =>
      validateTokenizer({ tokenizer: 'unicode61', tokenizerArgs: ['remove_diacritics', '1'] }, 'rebuild'),
    ).not.toThrow();
  });
  ...
  it('rejects a tokenizer arg with unsafe characters', () => {
    expect(() =>
      validateTokenizer({ tokenizer: 'unicode61', tokenizerArgs: ["1'; DROP TABLE x; --"] }, 'rebuild'),
