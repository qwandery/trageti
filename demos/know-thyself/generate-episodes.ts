...
function scorePath(path: string, added: number, deleted: number): number {
  ...
  if (path.startsWith('test/') && (path.includes('trajectory') || path.includes('semantic') || path.includes('graph'))) score += 250
...
async function buildInitialSource(
  ...
  const keyFiles = ['_docs/specs/trageti-spec-v0.1.md', 'src/domain/types.ts', 'src/store/TemporalStore.ts', 'README.md']
  const fileSections = keyFiles.map((path) => `### ${path}\n\n\`\`\`txt\n${fileAt(current, path, tokenBudget / keyFiles.length)}\n\`\`\``)
...
async function main(): Promise<void> {
  ...
  const episodes: Array<{ id: string; namespace: string; position: number; occurredAt: string; type: string; content: string }> = []
  ...
  console.log('')
  console.log('(review .local source documents, copy accepted files into demos/know-thyself/data/sources, then regenerate fixtures)')
}
