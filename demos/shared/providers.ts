...
export function createAnthropicExtractionProvider(apiKey: string, model = 'claude-sonnet-4-20250514'): ExtractionProvider {
  return {
    name: 'anthropic',
...
export function createOpenAICompatibleExtractionProvider(options: {
  ...
  return {
    ...
    async extract(prompt) {
      ...
      if (typeof content !== 'string') throw new Error('OpenAI-compatible extraction response missing choices[0].message.content')
      return content
    },
  }
}
...
export function createOpenAICompatibleEmbeddingProvider(options: {
  ...
  const provider: EmbeddingProvider = {
    ...
    async embed(texts: readonly string[], _embedOptions?: EmbedOptions): Promise<Float32Array[]> {
      ...
      return rows.map((row, i) => {
        ...
        if (!Array.isArray(embedding)) throw new Error(`OpenAI-compatible embedding response missing data[${String(i)}].embedding`)
  ...
  const provider: EmbeddingProvider = {
    ...
    async embed(texts: readonly string[], _embedOptions?: EmbedOptions): Promise<Float32Array[]> {
      ...
      for (const text of texts) {
        ...
        out.push(new Float32Array(embedding as number[]))
      }
      return out
    },
  }
...
function resolveExtractionProvider(provider: string, env: NodeJS.ProcessEnv): ExtractionProvider {
  ...
  if (provider === 'anthropic') {
    const apiKey = required(env['DEMO_EXTRACT_API_KEY'] ?? env['ANTHROPIC_API_KEY'], 'DEMO_EXTRACT_API_KEY or ANTHROPIC_API_KEY')
...
function resolveEmbeddingProvider(provider: string, env: NodeJS.ProcessEnv, dimension: number): DemoEmbeddingProvider {
  ...
  if (!Number.isFinite(resolvedDimension)) throw new Error(`DEMO_EMBED_DIMENSION must be numeric, got ${String(envDim)}`)
  if (provider === 'openai-compatible') {
  ...
  if (provider === 'fixture') throw new Error('Internal error: fixture embedding must be resolved with fixture vectors.')
  throw new Error(`Unsupported DEMO_EMBED_PROVIDER "${provider}". Use fixture, openai-compatible, or ollama-native.`)
}

function openAICompatPreset(env: NodeJS.ProcessEnv, purpose: 'extract' | 'embed'): {
  baseUrl: string
  apiKey: string
  defaultModel: string
  label: string
...
async function readJsonResponse(response: Response, label: string): Promise<unknown> {
  ...
  if (!response.ok) {
    throw new Error(`${label} failed HTTP ${String(response.status)} ${response.statusText}: ${text.slice(0, 500)}`)
  }
  try {
    return JSON.parse(text) as unknown
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    throw new Error(`${label} returned non-JSON response (${msg}): ${text.slice(0, 500)}`)
  }
}
