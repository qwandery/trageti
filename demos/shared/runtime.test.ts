...
describe('demoDataVersion', () => {
  ...
  it('is the same regardless of key insertion order (deterministic)', () => {
    const v1 = demoDataVersion('test-demo', BASE_EPISODES, BASE_FIXTURES, { 'b': [1], 'a': [2] }, BASE_QUERY_EMBEDDINGS, BASE_QUERY_TEXTS)
    const v2 = demoDataVersion('test-demo', BASE_EPISODES, BASE_FIXTURES, { 'a': [2], 'b': [1] }, BASE_QUERY_EMBEDDINGS, BASE_QUERY_TEXTS)
    expect(v1).toBe(v2)
  })
})
