...
//
// Usage: npx tsx demos/alex-place/generate-fixtures.ts [--write]
...
async function main(): Promise<void> {
  ...
  const reviewFixturesPath = resolve('demos/.local/alex-place/generated-fixtures.ts')
  const reviewEmbeddingsPath = resolve('demos/.local/alex-place/generated-embeddings.ts')
  ...
  if (writeCommitted) {
    writeGenerated(resolve('demos/alex-place/data/fixtures.ts'), fixturesContent)
    writeGenerated(resolve('demos/alex-place/data/embeddings.ts'), embeddingsContent)
    console.log('-> updated committed alex-place fixture files because --write was supplied')
  } else {
